...
The included `vercel.json` is set up for deploying the React frontend from the repository root. In Vercel, set:

```env
VITE_API_BASE_URL=https://your-backend-url/api
```

The Express backend also needs to be deployed with these environment variables:
...
MongoDB Atlas is recommended for a hosted database.
